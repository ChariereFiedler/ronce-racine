...
function shouldRewrite(cmd: string): boolean {
  ...
  if (!/^npm\s+(install|ci|i)((?:\s+-{1,2}\S+)*)\s*$/.test(trimmed)) return false
  // Already silent → we add nothing
  if (/--silent|--quiet|(^|\s)-s(\s|$)/.test(trimmed)) return false
  return true
}
...
function main(): void {
  ...
  if (!shouldRewrite(cmd)) process.exit(0)
